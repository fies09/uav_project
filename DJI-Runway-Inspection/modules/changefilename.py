    ...
    for old_name in old_names:      #遍历列表下的文件名
            if  old_name!= sys.argv[0]:  #代码本身文件路径，防止脚本文件放在path路径下时，被一起重命名
               if old_name.endswith('.JPG'):   #当文件名以.txt后缀结尾时
                    ...
                    logger.info(old_name,"has been renamed successfully! New name is: ", str(index)+".JPG")  #输出提示
                    index = index + 1
#对比文件夹
def contrast_floder(path1,path2,outpath):
    ...
    if len(names1) != len(names2):
        logger.info("两组图片对比失败，因为文件数量不统一")
        return
    index = 1
    for cur in names1:
        logger.info("开始执行当前文件："+str(index))
        ...
        if tag:
            logger.info("异常:" + path2 + "/" + str(index) + ".JPG")
            cv.imwrite(imgout_name, rectangle_img) # 将框住异物的图片保存
        else:
            logger.info("正常:"+path2 + "/" + str(index) + ".JPG")
        index = index + 1
